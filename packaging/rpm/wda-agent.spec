...
%description
Wazuh Desktop Agent (WDA) is a modular, cross-platform security agent
...
install -D -m 0644 config.yaml          %{buildroot}%{_sysconfdir}/wazuh-desktop-agent/config.yaml
install -D -m 0644 wda-agent.service    %{buildroot}%{_unitdir}/wda-agent.service
install -d                              %{buildroot}%{_sysconfdir}/wazuh-desktop-agent/sca
install -d                              %{buildroot}%{_sharedstatedir}/wazuh-desktop-agent
install -d                              %{buildroot}%{_localstatedir}/log/wazuh-desktop-agent

%files
%attr(0755, root, root)     %{_bindir}/wda-agent
%dir                        %{_sysconfdir}/wazuh-desktop-agent
%dir                        %{_sysconfdir}/wazuh-desktop-agent/sca
%config(noreplace)          %{_sysconfdir}/wazuh-desktop-agent/config.yaml
%{_unitdir}/wda-agent.service
%attr(0750, wda, wda) %dir  %{_sharedstatedir}/wazuh-desktop-agent
%attr(0750, wda, wda) %dir  %{_localstatedir}/log/wazuh-desktop-agent

%pre
getent group wda >/dev/null || groupadd -r wda
getent passwd wda >/dev/null || \
    useradd -r -g wda -d %{_sharedstatedir}/wazuh-desktop-agent \
            -s /sbin/nologin -c "Wazuh Desktop Agent" wda
exit 0
...
if [ "$1" -eq 0 ]; then
    ...
    rm -rf %{_sharedstatedir}/wazuh-desktop-agent \
           %{_localstatedir}/log/wazuh-desktop-agent
    getent passwd wda >/dev/null && userdel wda >/dev/null 2>&1 || :
    getent group  wda >/dev/null && groupdel wda >/dev/null 2>&1 || :
fi

%changelog
* Mon Apr 20 2026 Wazuh Desktop Agent Contributors <security@example.com> - 0.1.0-1
