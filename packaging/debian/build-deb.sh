#!/usr/bin/env bash
# Build a .deb package for wda-agent from an already-compiled release
# binary. Requires `dpkg-deb` (ships with `dpkg-dev` on Debian/Ubuntu).
#
# Usage:
#   BIN=target/release/wda-agent packaging/debian/build-deb.sh
#
# Output: dist/wda-agent_<version>_<arch>.deb
set -euo pipefail
...
if [ ! -x "$BIN" ]; then
    ...
    echo "       build it first with: cargo build --release -p wda-agent" >&2
    exit 1
fi
...
OUT="$OUT_DIR/wda-agent_${VERSION}_${ARCH}.deb"
