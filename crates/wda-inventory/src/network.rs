        ...
        Ok(ifaddrs) => {
            ...
            for ifaddr in &entries {
                ...
                if seen_ifaces.insert(name.clone()) {
                    ...
                    let state = read_interface_state(&name).unwrap_or_else(|| "unknown".to_string());
                ...
                if let Some(addr) = ifaddr.address {
                    if let Some(sock_addr) = addr.as_sockaddr_in() {
                        ...
                        let netmask = ifaddr
                            .netmask
                            .and_then(|n| n.as_sockaddr_in().map(|s| IpAddr::V4(s.ip()).to_string()))
                            .unwrap_or_default();
                        let broadcast = ifaddr
                            .broadcast
                            .and_then(|b| b.as_sockaddr_in().map(|s| IpAddr::V4(s.ip()).to_string()))
                            .unwrap_or_default();
                        ...
                        let netmask = ifaddr
                            .netmask
                            .and_then(|n| n.as_sockaddr_in6().map(|s| IpAddr::V6(s.ip()).to_string()))
                            .unwrap_or_default();
...
mod tests {
    ...
    fn test_collect_network_info_returns_results() {
        ...
        assert!(!payloads.is_empty(), "expected at least one network payload");
