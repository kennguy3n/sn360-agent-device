...
//!
//! Usage:
//!   cargo run --release --example burst_watcher -p wda-fim -- /tmp/wda-bench-fim
//!
//! The process watches the directory given on the command line (or
//! `/tmp/wda-bench-fim` if none is provided), drains every event it
...
async fn main() -> anyhow::Result<()> {
    let watch = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "/tmp/wda-bench-fim".to_string());
