...
//! [`UpdateConfig::public_key`](wda_core::config::UpdateConfig::public_key).
...
use wda_core::config::UpdateConfig;
...
pub async fn install_update(
    ...
    let temp_path = parent.join(format!(".wda-agent.update.{}", std::process::id()));
...
async fn download_and_verify(
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent(concat!("wda-agent-updater/", env!("CARGO_PKG_VERSION")))
        .build()
...
mod tests {
    ...
    fn backup_path_appends_bak() {
        let p = Path::new("/usr/bin/wda-agent");
        assert_eq!(backup_path_for(p), PathBuf::from("/usr/bin/wda-agent.bak"));
    }
    ...
    fn rollback_restores_previous_binary() {
        ...
        let current = dir.path().join("wda-agent");
        let backup = dir.path().join("wda-agent.bak");
