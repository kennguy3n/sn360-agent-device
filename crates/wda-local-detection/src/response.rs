...
//!   `wda-active-response`, which drives the platform-native firewall.
...
use wda_active_response::actions::{ActionParams, ActionRegistry};
use wda_core::config::LocalDetectionConfig;
...
/// Thin wrapper around the `wda-active-response` registry plus the
